...
public interface PaymentMethodRepository extends CrudRepository<PaymentMethod, Integer> {
  ...
  List<CreditCard> findAllCreditCardsForUser(@Param("email") String email);
  ...
  List<BankAccount> findAllBankAccountsForUser(@Param("email") String email);
  ...
  boolean bankAccountExists(@Param("email") String email, @Param("iban") String iban);
  ...
  boolean creditCardExists(@Param("email") String email, @Param("cardNumber") Long cardNumber);

  @Query("SELECT P FROM Account A LEFT JOIN A.paymentMethods P where P.id=:id and A.email=:email")
  PaymentMethod findPaymentMethodByIdFromUser(@Param("email") String email, @Param("id") int id);
}
